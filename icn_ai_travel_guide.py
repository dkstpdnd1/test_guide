...
def remove_airport_suffix(city):
    city = clean_destination_text(city)

    remove_words = [
        "나리타", "하네다", "수완나품", "샤를드골", "타오위안",
        "로스앤젤레스", "두바이", "도하"
    ]

    # 도쿄/나리타 같은 경우 도쿄 중심 검색
    if "도쿄" in city:
        return "도쿄"
    if "방콕" in city:
        return "방콕"
...
def is_relevant_local_info(item):
...
def naver_search(query, display=10, sort="sim", search_type="blog"):
    ...
    url = url_map.get(search_type, url_map["blog"])
...
def get_naver_image(query):
    items = naver_search(query, display=5, sort="sim", search_type="image")
...
def convert_naver_item(item, source_type, city_query):
    ...
    link = (
        ...
        or item.get("bloggerlink")
        or item.get("mapx")
        or ""
    )

    published = item.get("postdate", "") or item.get("pubDate", "")

    image_url = get_article_image(link)

    if not image_url:
        image_url = get_naver_image(f"{city_query} {title}")
...
@st.cache_data(ttl=1800)
def get_destination_contents(city, country, max_items=6):
    """
    항공사/공항 관련 결과는 제외하고,
    실제 여행 활용 정보 중심으로 네이버에서 검색.
    """

    city_query = remove_airport_suffix(city)

    search_queries = [
        f"{city_query} 여행 코스",
        f"{city_query} 맛집 추천",
        f"{city_query} 가볼만한곳",
        f"{city_query} 현지 여행 팁",
        f"{city_query} 요즘 핫플",
        f"{city_query} 축제 전시 공연"
    ]
    ...
    for query in search_queries:
        blog_items = naver_search(query, display=8, sort="sim", search_type="blog")

        for item in blog_items:
            converted = convert_naver_item(item, "블로그", city_query)

            if not converted["link"] or converted["link"] in seen_links:
                continue

            if not is_relevant_local_info(converted):
                continue

            seen_links.add(converted["link"])
...
@st.cache_data(ttl=1800)
def get_destination_trend_news(city, country, max_items=5):
    """
    지역 트렌드/행사/문화/사회 뉴스 위주.
    항공·공항 관련 뉴스 제외.
    """
    ...
    for query in search_queries:
        news_items = naver_search(query, display=8, sort="date", search_type="news")

        for item in news_items:
            converted = convert_naver_item(item, "뉴스", city_query)
...
def make_rule_based_summary(row, news_items, travel_items):
    country = row["도착 국가"]
    city = row["도착 도시"]
    city_query = remove_airport_suffix(city)
    flight = row["항공편명"]
    ...
    trend_title = news_items[0]["title"] if news_items else "관련 트렌드 정보 없음"
    travel_title = travel_items[0]["title"] if travel_items else "관련 여행 정보 없음"

    return f"""
### ✈️ 맞춤 여행 가이드 요약

현재 선택된 항공편은 **{flight}**이며, 도착지는 **{city_query} / {country}**입니다.

#### 1. 지금 공항에서 해야 할 일
출발 터미널은 **{row['출발 터미널']}**입니다.  
수하물이 있다면 체크인 카운터를 먼저 이용하고, 수하물이 없다면 보안검색대로 이동할 수 있습니다.
...
def make_llm_summary(row, news_items, travel_items):
    api_key = os.getenv("OPENAI_API_KEY")

    if not api_key or OpenAI is None:
        return make_rule_based_summary(row, news_items, travel_items)
...
{make_rule_based_summary(row, news_items, travel_items)}
...
news_count = st.sidebar.slider("지역 트렌드 뉴스 개수", 3, 10, 5)
travel_count = st.sidebar.slider("여행 정보 개수", 3, 10, 6)

if not os.getenv("NAVER_CLIENT_ID") or not os.getenv("NAVER_CLIENT_SECRET"):
    st.warning(
        "네이버 검색 API 키가 설정되지 않았습니다. "
        "PowerShell에서 NAVER_CLIENT_ID, NAVER_CLIENT_SECRET 환경변수를 설정하세요."
    )

city_query = remove_airport_suffix(row["도착 도시"])
country_query = clean_destination_text(row["도착 국가"])

trend_news_items = get_destination_trend_news(city_query, country_query, news_count)
travel_items = get_destination_contents(city_query, country_query, travel_count)
...
if st.button("여행 가이드 생성", type="primary"):
    with st.spinner("여행 가이드를 생성 중입니다..."):
        summary = make_llm_summary(row, trend_news_items, travel_items)
        st.markdown(summary)
...
st.subheader(f"📰 {city_query} 지역 트렌드 뉴스")
st.caption("항공사/공항 관련 정보는 제외하고, 관광·축제·전시·맛집·현지 트렌드 중심으로 표시합니다.")

if trend_news_items:
    for item in trend_news_items:
        render_info_card(item)
else:
    st.info("지역 트렌드 뉴스를 찾지 못했습니다. 네이버 API 키 또는 검색어를 확인하세요.")
...
st.subheader(f"🌍 {city_query} 여행 활용 정보")
st.caption("여행 코스, 맛집, 가볼만한 곳, 현지 팁 중심으로 표시합니다.")

if travel_items:
    for item in travel_items:
        render_info_card(item)
else:
    st.info("여행 정보를 찾지 못했습니다. 네이버 API 키 또는 검색어를 확인하세요.")
